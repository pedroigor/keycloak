...
We recommend that you switch to the Elytron OIDC Java adapter to secure your applications.
This adapter has a similar configuration to Keycloak Java adapters, so you can expect a smooth migration of your applications.
...
We do not regularly test and maintain WildFly/EAP adapters. They may work only with WildFly version 23 or earlier or with JBoss EAP 7. Also note
that they may work only with JEE applications, but not with JakartaEE applications.
====
