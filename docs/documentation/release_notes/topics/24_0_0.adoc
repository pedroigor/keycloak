...
In this release, we are encapsulating the root user attributes (e.g.: `username`, `email`, `firstName`, `lastName`, and `locale`) by moving them to a base/abstract class in order to align how these attributes
are marshalled and unmarshalled when using both Admin and Account REST APIs.

By doing that, we expect more consistency on how attributes are managed by clients as well as make sure they are in conformance to the user profile
configuration set to a realm.

For more details, check the link:{upgradingguide_link}[{upgradingguide_name}].
