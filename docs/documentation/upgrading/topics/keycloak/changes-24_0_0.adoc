...
The following changes were done in the `org.keycloak.userprofile.Attributes` interface:
...
so that the root user attributes (e.g.: `username`, `email`, `firstName`, `lastName`, and `locale`) have a consistent representation when fetching or sending
...
In addition to that, the `getAttributes` method is targeted for representing only custom attributes, so you shouldn't expect any root attribute in the map returned by this method. This method is
