...
The JavaScript file must define a `render` function that will be called everytime the page is rendered. For instance, a bare minimal
JavaScript file looks like the following:
...
You can also force the page to re-render and re-execute any script associated with `kc` annotations by calling the `renderAttributes` function. By calling this
function, all the `render` methods from the JavaScript files associated with `kc` annotations will be invoked.
