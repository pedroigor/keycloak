...
public class UserStoragePrivateUtil {
    ...
    public static SynchronizationResult runFullSync(KeycloakSessionFactory sessionFactory, UserStorageProviderModel provider) {
        return KeycloakModelUtils.runJobInTransactionWithResult(sessionFactory, session -> new UserStorageSyncTask(provider, SyncMode.FULL).runWithResult(session));
    }
    ...
    public static SynchronizationResult runPeriodicSync(KeycloakSessionFactory sessionFactory, UserStorageProviderModel provider) {
        return KeycloakModelUtils.runJobInTransactionWithResult(sessionFactory, session -> new UserStorageSyncTask(provider, SyncMode.CHANGED).runWithResult(session));
    }
}
