...
@NamedQueries(
        {
                ...
                @NamedQuery(name="findPolicyIdByResourceType", query="select p from PolicyEntity p inner join p.config c left join fetch p.associatedPolicies a left join p.resources r where p.resourceServer.id = :serverId and ((KEY(c) = 'defaultResourceType' and c like :type) or (r.resourceServer = :serverId and r.type like :type))"),
