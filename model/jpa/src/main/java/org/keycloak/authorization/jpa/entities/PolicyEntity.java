...
@NamedQueries(
        {
                ...
                @NamedQuery(name="findPolicyIdByResourceNoScope", query="select p from PolicyEntity p inner join fetch p.scopes s inner join p.resources r where p.resourceServer.id = :serverId and (r.resourceServer = :serverId and r.id = :resourceId) and p.scopes is empty"),
                ...
                @NamedQuery(name="findPolicyIdByNullResourceType", query="select p from PolicyEntity p inner join p.config c inner join fetch p.associatedPolicies a where p.resourceServer.id = :serverId and p.resources is empty and KEY(c) = 'defaultResourceType' and c like :type"),
