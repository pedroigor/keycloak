...
public class ClientAttributeCertificateResource {
    ...
    private CertificateRepresentation getCertFromRequest() throws IOException {
        ...
        MultivaluedMap<String, FormPart> uploadForm = session.getContext().getHttpRequest().getMultiPartFormParameters();
        FormPart keystoreFormatPart = uploadForm.getFirst("keystoreFormat");
        if (keystoreFormatPart == null) throw new BadRequestException();
        String keystoreFormat = keystoreFormatPart.getValue();
        FormPart inputParts = uploadForm.getFirst("file");
        if (keystoreFormat.equals(CERTIFICATE_PEM)) {
            String pem = StreamUtil.readString(inputParts.getInputStream());
            ...
            InputStream stream = inputParts.getInputStream();
        ...
        String keyAlias = uploadForm.getFirst("keyAlias").getValue();
        FormPart keyPasswordPart = uploadForm.getFirst("keyPassword");
        char[] keyPassword = keyPasswordPart != null ? keyPasswordPart.getValue().toCharArray() : null;

        FormPart storePasswordPart = uploadForm.getFirst("storePassword");
        char[] storePassword = storePasswordPart != null ? storePasswordPart.getValue().toCharArray() : null;
        PrivateKey privateKey = null;
        X509Certificate certificate = null;
        try {
            KeyStore keyStore = CryptoIntegration.getProvider().getKeyStore(KeystoreFormat.valueOf(keystoreFormat));
            keyStore.load(inputParts.getInputStream(), storePassword);
            try {
