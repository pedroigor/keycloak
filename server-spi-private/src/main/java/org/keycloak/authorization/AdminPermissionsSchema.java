...
public class AdminPermissionsSchema extends AuthorizationSchema {
    ...
    public void resourceRepresentationValidation(KeycloakSession session, ResourceServer resourceServer, Resource resource, ResourceRepresentation rep) {
        if (isAdminPermissionClient(session.getContext().getRealm(), resourceServer.getId())) {
            if (rep.getType() == null || SCHEMA.getResourceTypes().get(rep.getType()) == null) {
                throw new ModelValidationException("Resource type not provided or not found in schema.");
            }
            if (SCHEMA.getResourceTypes().get(resource.getName()) != null) {
                throw new ModelValidationException("Updates not supported for 'all-resource-type' resources.");
            }
            if (rep.getOwner() != null && !rep.getOwner().getId().equals(resourceServer.getClientId())) {
                throw new ModelValidationException("Owner should reference admin permission client.");
            }
            ...
            if (!schemaScopes.containsAll(resourceScopes)) {
                throw new ModelValidationException("Unexpected scopes found. Expected scopes: " + schemaScopes);
            }

            // fields below are not expected, do not store 
