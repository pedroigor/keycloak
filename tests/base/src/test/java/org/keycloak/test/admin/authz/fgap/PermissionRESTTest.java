...
public class PermissionRESTTest extends AbstractPermissionTest {
    ...
    public void resourcesTest() {
        ...
        try (Response response = client.admin().authorization().resources().resource(usersResource.getId()).update(resourceRep)) {
            assertThat(response.getStatus(), equalTo(Response.Status.BAD_REQUEST.getStatusCode()));
        }
        ...
        // no resourceType
        userAliceResourceRep.setType(null);
        try (Response response = client.admin().authorization().resources().resource(aliceResourceId).update(userAliceResourceRep)) {
            assertThat(response.getStatus(), equalTo(Response.Status.BAD_REQUEST.getStatusCode()));
        }

        // non-existent resourceType
        userAliceResourceRep.setType("custom");
        try (Response response = client.admin().authorization().resources().resource(aliceResourceId).update(userAliceResourceRep)) {
            assertThat(response.getStatus(), equalTo(Response.Status.BAD_REQUEST.getStatusCode()));
        }

        // not expected owner
        userAliceResourceRep.setType(AdminPermissionsSchema.USERS.getType());
        String owner = userAliceResourceRep.getOwner().getName();
        userAliceResourceRep.setOwner("custom_owner");
        try (Response response = client.admin().authorization().resources().resource(aliceResourceId).update(userAliceResourceRep)) {
            assertThat(response.getStatus(), equalTo(Response.Status.BAD_REQUEST.getStatusCode()));
        }

        // non-existent scopes
        userAliceResourceRep.setOwner(owner);
        userAliceResourceRep.setScopes(Set.of("manage", "edit", "token-exchange").stream().map(ScopeRepresentation::new).collect(Collectors.toSet()));
        try (Response response = client.admin().authorization().resources().resource(aliceResourceId).update(userAliceResourceRep)) {
            assertThat(response.getStatus(), equalTo(Response.Status.BAD_REQUEST.getStatusCode()));
        }

        try (Response response = client.admin().authorization().resources().resource(aliceResourceId).remove()) {
